...
app.get("/talk", async (req,res) => { //채팅 리스트 조회
  ...
  let [data] = await conn.query("SELECT a.*, b.member_no AS seller_no FROM talk_room AS a, member AS b WHERE (seller_id = ? OR buyer_id = ?) AND a.seller_id = b.member_id ;",[loginId,loginId]);
...
app.io.on('connection', function(socket) {
  ...
  socket.on('message', (data) => {
    ...
    conn.query("INSERT INTO talk_message (send_id, receive_id, message_text, message_time, talk_no) values(?, ?, ?, ?, ?)",[socket.sendId, socket.recevieId, data.message ,"12시", socket.roomNo],(err) => { //메세지 내용 DB 저장
      if(err) throw err;
    })
