...
export default {
    ...
    mounted() {
        this.getProductInfo(); // 상품 기존 정보 불러오기
        this.getCategoryInfo(); // 카테고리 정보 불러오기
	},
	methods:{
        /* 상품 정보 조회 */
        async getProductInfo(){
            try{
                ...
                const productInfo = res.data.productInfo[0];
                /* 상품정보 */
                this.title = productInfo.title;
                this.price = productInfo.price;
                this.state = productInfo.state;
                this.content = productInfo.content;
                this.selectLargeName = productInfo.category_large_name;
                this.selectMediumName = productInfo.category_medium_name;
                this.area = productInfo.area;
                /* 기존 이미지 */
                this.remainImage = productInfo.image_name.split(',');
                /* 미리보기 이미지 */
        ...
        /* 상품 카테고리 정보 조회 */
        async getCategoryInfo() { // 카테고리 데이터 불러오기
            try{
        ...
        async update(){  
            try{
                ...
                /* 업로드 POST */
                const res = await axios.post("http://localhost:3000/api/board/update/" + productNo ,frm, config)
